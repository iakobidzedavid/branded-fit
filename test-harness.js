...
async function testBrandExtraction(domain) {
    const startTime = Date.now();
    try {
        const result = await makeRequest("POST", "/api/v1/brand/extract", { domain });
        const duration = Date.now() - startTime;
        if (result.status === 200) {
            ...
            return {
                ...
                response: data,
                confidence: data.extraction_confidence_pct || 0,
            };
        }
        else {
            return {
...
async function testMockupGeneration(domain, extractionResult) {
    const startTime = Date.now();
    try {
        ...
        const extractData = extractionResult.response;
        const colors = extractData.colors || [];
        const logos = extractData.logos || [];
        const result = await makeRequest("POST", "/api/v1/mockup/generate", {
            domain,
        ...
        if (result.status === 200) {
            ...
            return {
                ...
                duration_ms: duration,
                response: data,
                product_count: data.sku_count || 0,
...
async function testStorefrontCreation(domain, mockupResult) {
    const startTime = Date.now();
    try {
        ...
        const mockupData = mockupResult.response;
        const productCount = mockupData.sku_count || 5;
        ...
        const result = await makeRequest("POST", "/api/v1/storefront/create", {
            domain,
...
async function testDomain(domain) {
    ...
    const all_stages_passed = brandExtraction.status === "success" &&
...
`;
    ...
    const allErrors = results.flatMap((r) => r.errors.map((e) => `- **${r.domain}:** ${e}`));
