...
async function waitForPipelineCompletion(page: Page, maxWaitMs: number = MAX_WAIT_TIME): Promise<PipelineState | null> {
    ...
    try {
      ...
      const state = await page.evaluate(() => {
        ...
        stageElements.forEach((el) => {
          ...
          const status = (statusEl?.getAttribute('data-status') || 'pending') as PipelineState['status'];
